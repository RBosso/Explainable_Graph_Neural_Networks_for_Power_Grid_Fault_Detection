...
layer_gc = LayerGradCam(model, model.conv3)#.conv3
...
attr = layer_gc.attribute(input, target = label1, additional_forward_args=(Edge_Index,Batch), relu_attributions=True)
print(attr)
print(attr.shape)
...
# print("Model Prediction = ",model(input, Edge_Index, Batch).argmax(dim=1).item())

# Thinking of ways to visualize these interpretations, in terms of validity as well (to implement later perhaps, but there other things that must be understood here)
...
BusesInOrder = ['SOURCEBUS', '800', '802', '806', '808', '810', '812', '814', '814R', '850', '816', '818', '824', '820', '822', '826', '828', '830', '854', '832', '858', '834', 
 '860', '842', '836', '840', '862', '844', '846', '848', '852R', '888', '856', '852', '864', '838', '890']

 #What the model actually predicts:
print("Model Prediction Label = ",model(input, Edge_Index, Batch).argmax(dim=1).item())
print("Model Fault Location Prediction = ",confusion_mat_labels[model(input, Edge_Index, Batch).argmax(dim=1).item()])
print()
# len(BusesInOrder)
visualize_attr = attr.squeeze().detach().numpy()
# print(visualize_attr)
print("Actual Label = ", label1)
print("Actual Fault Location = ",confusion_mat_labels[label1])

for n in range(len(BusesInOrder)):
    print(BusesInOrder[n],visualize_attr[n])
